...
import { Order, Cart } from './orderTypes';
...
export default async function orderRoutes(server: FastifyInstance) {
	...
	// POST shopping cart
	server.post('/cart/:id', { preHandler: [server.authenticate_self] }, async (request, reply) => {
	...
	server.delete('/cart/:id', { preHandler: [server.authenticate_self] }, async (request, reply) => {
		const { id } = request.params as { id: string };
